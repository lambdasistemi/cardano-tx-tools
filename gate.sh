#!/usr/bin/env bash
# Pre-push and pre-commit gate for PR backing issue #90
# (DSL-reconstructed amaru-disburse fixtures + typed emit follow-up scope).
# Subagents MUST run ./gate.sh and observe success before returning a commit.
# Removed in the final `chore(090): drop gate.sh (ready for review)` commit
