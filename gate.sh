#!/usr/bin/env bash
# Pre-push and pre-commit gate for PR backing issue #80
# (real Conway amaru disburse fixtures + RFC 6901 blueprint $ref normalization).
# Subagents MUST run ./gate.sh and observe success before returning a commit.
# Removed in the final `chore(080): drop gate.sh (ready for review)` commit
