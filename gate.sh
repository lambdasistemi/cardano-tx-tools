#!/usr/bin/env bash
# Pre-push and pre-commit gate for PR backing issue #95
# (typed-emit walker extension for OpenArray-of-OpenObject map entries).
# Worker pairs MUST run ./gate.sh and observe success before returning a
# commit. Removed in the final `chore(095): drop gate.sh (ready for review)`
