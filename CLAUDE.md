...
- Haskell, GHC 9.12.3 via `haskell.nix` (existing pin, constitution Operational Constraints). + No change. The engine edits live in `src/Cardano/Tx/Diff.hs`, which already pulls `aeson`, `yaml`, `cardano-ledger-*`, `text`, `bytestring`. `Cardano.Tx.Rewrite` is unchanged (it only re-exports the affected types). (040-collapse-nested-raw-omit)
- None at runtime. Test fixtures on disk under `test/fixtures/amaru-treasury-swap/` (existing) and `test/fixtures/mainnet-txbuild/swap-cancel-issue-8/` (existing). (040-collapse-nested-raw-omit)
...
## Recent Changes
- 040-collapse-nested-raw-omit: Added Haskell, GHC 9.12.3 via `haskell.nix` (existing pin, constitution Operational Constraints). + No change. The engine edits live in `src/Cardano/Tx/Diff.hs`, which already pulls `aeson`, `yaml`, `cardano-ledger-*`, `text`, `bytestring`. `Cardano.Tx.Rewrite` is unchanged (it only re-exports the affected types).
