...
- Haskell, GHC 9.12.3 via `haskell.nix` + existing `cardano-ledger-*`, `cardano-node-clients`, `microlens`, `containers`, `aeson` (061-seed-rewards-state)
...
## Recent Changes
- 061-seed-rewards-state: Added Haskell, GHC 9.12.3 via `haskell.nix` + existing `cardano-ledger-*`, `cardano-node-clients`, `microlens`, `containers`, `aeson`
