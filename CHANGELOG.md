...
* **graph:** type-driven exhaustive `ConwayDiffValue` body-walker coverage. Every body-reachable constructor has a positive case-arm; the compiler (`-Wincomplete-patterns` + `-Werror`) is the load-bearing exhaustivity gate. Closes the operator's 2026-05-21 crash mode (`UnsupportedLeafType: ConwayRequiredSignersValue`) at the type level. Required signers emit as `cardano:hasRequiredSigner` Identifier-typed bnodes; `cardano:totalCollateral` + `cardano:hasCollateralReturn` (Output sub-block) cover the second pre-existing fail-loudly leaf; voting procedures emit with `cardano:VoterDRep` / `VoterStakePool` / `VoterCommitteeCold` discrimination + `cardano:hasVerdict` literal + optional `cardano:hasAnchor`. Reference scripts type as `cardano:PlutusScript` (with `cardano:hasVersion` 1/2/3/4) or `cardano:NativeScript`. Every Conway cert + proposal variant has a positive case-arm — modeled ones (`StakeDelegation`, `VoteDelegation`, `TreasuryWithdrawals`) keep the rich shape, the rest emit a typed `cardano:Certificate, cardano:OpaqueLeaf` / `cardano:Datum` fallback with `cardano:leafType` discriminator + `cardano:hasRawBytes` CBOR-hex. Witness-set + redeemer coverage closes the seaboard contract — see the witness-set walker entry below.
* **graph:** identity-bearing predicates emit as `cardano:Identifier`-typed bnodes, not literals. `cardano:hasRequiredSigner` / `cardano:scriptDataHash` / `cardano:auxiliaryDataHash` / `cardano:hasHash` (Datum) / `cardano:hasHash` (ReferenceScript) now bind to `_:cred_*` / `_:hash_*` bnodes carrying `a cardano:Identifier ; cardano:leafType "X" ; cardano:bytesHex "<hex>"` — joinable across positions in SPARQL views without IRI surgery. `cardano:fromTxOutRef` decomposed into a typed `cardano:TxOutRef` sub-node with `cardano:hasTxId` Identifier + `cardano:hasIndex` integer.
* **graph:** `Cardano.Tx.Graph.Emit.VocabExport.renderVocabFragment` derives the canonical Turtle fragment from `Vocab.hs`. `VocabExportSpec` enforces sync between `Vocab.hs` and `test/fixtures/canonical-vocab/derived.ttl`; the derived fragment is the authoritative source for upstream kmaps publication.
* **graph:** `BlockfrostSampleSmokeSpec` terminal acceptance gate. Every `*.cbor.hex` file under `test/fixtures/blockfrost-cache/` runs through `decodeConwayTxInput + emit + serialize` and must produce non-empty Turtle. Closes the operator's acceptance rule (random Conway mainnet txs emit cleanly) at the real-chain layer; the operator's 2026-05-21 morning tx is vendored as the first cache entry.
* **graph:** strict canonical-vocab traceability gate (`VocabTraceabilitySpec`). Every emitted `cardano:*` CURIE must trace to a declaration in the pinned canonical fragment. Companion kmaps PR [lambdasistemi/cardano-knowledge-maps#56](https://github.com/lambdasistemi/cardano-knowledge-maps/pull/56) merged at kmaps@cfb599b7 for the 17 classes + 16 properties (Phase A.2). Previous Phase A.1 pin (kmaps#55 @ cce9625b) retained as the merge base.
...
* **graph:** witness-set walker — seaboard-complete emission of redeemers, key / datum / script / bootstrap witnesses. New top-level edges on `_:tx`: `cardano:hasRedeemer` / `cardano:hasKeyWitness` / `cardano:hasDatumWitness` / `cardano:hasScriptWitness` / `cardano:hasBootstrapWitness`. Redeemers carry `cardano:hasPurpose` (`"Spend"`|`"Mint"`|`"Cert"`|`"Withdraw"`|`"Propose"`|`"Vote"`) + `cardano:hasIndex` + `cardano:hasData` (→ `cardano:Datum`, opaque CBOR until [#50](https://github.com/lambdasistemi/cardano-tx-tools/issues/50)) + `cardano:hasExUnits` (→ `cardano:ExUnits` with `cardano:memoryUnits` + `cardano:cpuUnits` literals). Key witnesses carry `cardano:hasSignature` (Ed25519 hex literal) + `cardano:hasVerificationKey` → shared `cardano:Identifier` (`PaymentKey`) bnode 1-hop SPARQL-joinable with body `cardano:hasRequiredSigner`. Datum witnesses reuse the `cardano:Datum` bnode-identity (joinable with output inline datums by hash); script witnesses reuse the `cardano:PlutusScript` / `cardano:NativeScript` bnode-identity (joinable with output `cardano:hasReferenceScript` by hash); bootstrap witnesses as `cardano:BootstrapWitness` + `cardano:OpaqueLeaf` rawBytes. Compiler-as-exhaustivity gate (`-Wincomplete-patterns` + `-Werror`) restored to full load-bearing role over the entire `ConwayTx`, not just a body-reachable subset. Closes [#78](https://github.com/lambdasistemi/cardano-tx-tools/issues/78) ([#77](https://github.com/lambdasistemi/cardano-tx-tools/pull/77)).
* **graph:** Blockfrost smoke gate strengthened — asserts the operator-paste mainnet swap-order tx (`test/fixtures/blockfrost-cache/operator-paste-2026-05-21.cbor.hex`) emits the redeemer block. The seven witness-set predicates (`cardano:hasRedeemer`, `cardano:hasPurpose`, `cardano:hasIndex`, `cardano:hasData`, `cardano:hasExUnits`, `cardano:memoryUnits`, `cardano:cpuUnits`) plus the value-content checks `cardano:hasPurpose "Spend"` + `cardano:hasIndex 0` are pinned against the live-cached real-chain bytes. Fixtures with no redeemers fall through to `pendingWith`. Closes the v3 acceptance criterion ("Operator paste tx emits redeemer block").
* **graph:** Phase A.3 kmaps additions staged — the witness-set walker emits 15 net-new `cardano:` CURIEs (`cardano:KeyWitness` / `cardano:BootstrapWitness` / `cardano:ExUnits` classes; `cardano:hasRedeemer` / `hasKeyWitness` / `hasDatumWitness` / `hasScriptWitness` / `hasBootstrapWitness` / `hasPurpose` / `hasData` / `hasExUnits` / `memoryUnits` / `cpuUnits` / `hasSignature` / `hasVerificationKey` properties) that the prior canonical pin (kmaps@cfb599b7 post-Phase-A.2) does not declare. Short-circuited via `VocabTraceabilitySpec.pendingPhaseA3` until the parallel kmaps#57 Phase A.3 PR merges and the pin refreshes. `cardano:Redeemer` was already canonical from Phase A.2 — no re-issue.
* **docs:** asciinema cast re-recorded against the post-T122c node-shape — six sections covering `fromTxOutRef` decomposition, Identifier-typed bnodes for credentials / hashes, script-language discrimination, body-root predicates, mint signed quantity, and an end-to-end run on the operator-paste tx from the Blockfrost cache.
* **docs:** asciinema cast re-recorded again — adds Section 7 (witness-set redeemer block on the operator-paste swap-order tx: `hasPurpose "Spend"`, `hasIndex 0`, `hasData`, `hasExUnits` with `mem=438877` / `cpu=147488633`) and Section 8 (key witness shared-bnode invariant: `_:cred_paymentkey_<hex>` bound to both `hasRequiredSigner` and `hasVerificationKey`, 1-hop SPARQL join with no string surgery). `docs/assets/asciinema/tx-graph.cast` 12815 bytes.
