...
- Q: What top-level YAML shape does a unified rewriting-rules document use, given that existing collapse-only YAML must parse unchanged? → A: Top-level keys `collapse:` and `rename:`, each carrying a homogeneous list of rules for that stage. The new loader ALSO accepts the legacy bare-list shape (a top-level list of `CollapseRule` objects) by wrapping it under `collapse:` internally, so every existing collapse-only YAML file parses unchanged. Either section may be omitted; an empty/missing section means "no rules for that stage". Stage order is enforced by the engine and is independent of the order of keys in the document.
...
- **FR-007**: `tx-inspect --rules PATH` MUST load a unified rewriting-rules YAML document with two optional top-level sections: `collapse:` (a list of `CollapseRule` entries, identical to the existing collapse-rules format) and `rename:` (a list of `RenameRule` entries). The new loader MUST ALSO accept the legacy bare-list shape — a top-level list of `CollapseRule` objects, no `collapse:` key — by wrapping it under `collapse:` internally, so every existing collapse-only YAML file that parses today via `parseCollapseRulesYaml` parses unchanged. Either section may be omitted; a missing or empty section means "no rules for that stage". The order of the `collapse:` / `rename:` keys in the document MUST NOT affect rendering — stage order is engine-enforced, not document-order-driven.
