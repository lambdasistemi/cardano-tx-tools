...
| II. Module Namespace Discipline | PASS | New module `Cardano.Tx.Rewrite.RewriteRedesignGoldenSpec` and (optional) `Cardano.Tx.Rewrite.Harness.Helpers` are under the `Cardano.Tx.*` namespace. No `Node.Client.*` introduction. |
