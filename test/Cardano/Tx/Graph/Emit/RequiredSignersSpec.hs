...
spec = describe "Cardano.Tx.Graph.Emit required signers (T116 / S15)" $ do
    ...
    it "emits one cardano:hasRequiredSigner per signer (single)" $ do
        let bytes = emitBytes (baseTx & requiredSigners [stubKeyHash 0xaa])
        bytes
            `shouldSatisfy` BS8.isInfixOf
                ( "cardano:hasRequiredSigner \""
                    <> BS8.replicate 56 'a'
                    <> "\""
                )
    it "emits one cardano:hasRequiredSigner per signer (multiple, sorted)" $ do
        let bytes =
                emitBytes
                    ( baseTx
                        & requiredSigners
        ...
        bytes
            `shouldSatisfy` BS8.isInfixOf
                ("cardano:hasRequiredSigner \"" <> hexA <> "\"")
        bytes
            `shouldSatisfy` BS8.isInfixOf
                ("cardano:hasRequiredSigner \"" <> hexB <> "\"")
        bytes
            `shouldSatisfy` BS8.isInfixOf
                ("cardano:hasRequiredSigner \"" <> hexC <> "\"")
        -- Ascending byte order: 'a' < 'b' < 'c'.
        let posA = BS8.length (fst (BS8.breakSubstring hexA bytes))
            posB = BS8.length (fst (BS8.breakSubstring hexB bytes))
            posC = BS8.length (fst (BS8.breakSubstring hexC bytes))
